...
export default function PopoverWorks({ data }) {
  return (
            ...
            {data.items.map((item, i) => (
              ...
              >
                <div className="flex h-9 w-9 sm:w-11 sm:h-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                  <item.icon className="h-5 w-5 sm:w-6 sm:h-6 text-gray-600 group-hover:text-green-700" aria-hidden="true" />
                </div>
                <div className="flex-auto">
                  <a href={item.href} className="block text-gray-900 font-regular text-base font-medium">
                    {item.name}
